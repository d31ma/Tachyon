...
| **Windows native accessibility** | The Win32 gate ran for the first time on 2026-08-01 and every child control reaches UI Automation as `ControlType.Pane` with only its name, rather than as a button, output, or heading. The generated host does create real `BUTTON`, `EDIT`, and `STATIC` controls, and the side-by-side manifest was being published where Windows never reads it — corrected — but that did not change the reported control types. The button therefore exposes no invoke pattern, so the Phase 5 accessibility and interaction claims are unevidenced on Windows. This is a real product gap, not a harness defect. | Identifying why the standard-control proxies do not annotate this process, then a passing `Phase 5 Windows Win32 native gate`. |
