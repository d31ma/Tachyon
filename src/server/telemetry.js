...
class TelemetryConfig {
    static collectionName = process.env.OTEL_COLLECTION || 'otel-spans';
     ...
     */
    static enabled() {
        return TelemetryConfig.isTruthy(process.env.OTEL_ENABLED);
    }
     ...
     */
    static root() {
        return process.env.OTEL_FYLO_ROOT || path.join(process.cwd(), '.tachyon-otel');
    }
     ...
     */
    static serviceName() {
        return process.env.OTEL_SERVICE_NAME || '@d31ma/tachyon';
    }
    ...
    static serviceVersion() {
        return process.env.OTEL_SERVICE_VERSION || process.env.npm_package_version || '2.0.0';
    }
    ...
    static shouldCaptureIp() {
        return TelemetryConfig.isTruthy(process.env.OTEL_CAPTURE_IP);
    }
}
...
class FyloTelemetryStore extends TelemetryStore {
    ...
    async getFylo() {
        if (!FyloTelemetryStore.fyloPromise) {
            FyloTelemetryStore.fyloPromise = Promise.resolve(new Fylo({
                root: TelemetryConfig.root(),
            }));
        }
