...
export default class Pool {
    ...
    /**
     * Extensionless handler files spawn cleanly on Unix via shebang resolution,
     * but Windows needs the interpreter command explicitly. We derive that
     * interpreter from the file's shebang when present.
    ...
    static resolveHandlerCommand(handler) {
        ...
        if (!firstLine.startsWith('#!'))
            return [handler];
        let tokens = tokenizeCommand(firstLine.slice(2).trim());
        if (tokens.length === 0)
            return [handler];
