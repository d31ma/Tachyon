...
export default class Router {
    ...
    static routeSlugs = {};
    static routesPath = Router.resolveWorkspacePath('ROUTES_PATH', ['server/routes', 'routes']);
    static pagesPath = Router.resolveWorkspacePath('PAGES_PATH', ['browser/pages', 'routes']);
    static componentsPath = Router.resolveWorkspacePath('COMPONENTS_PATH', ['browser/components', 'components']);
    static assetsPath = Router.resolveWorkspacePath('ASSETS_PATH', ['browser/shared/assets', 'shared/assets', 'assets']);
    static sharedDataPath = Router.resolveWorkspacePath('SHARED_DATA_PATH', ['browser/shared/data', 'shared/data']);
    static sharedScriptsPath = Router.resolveWorkspacePath('SHARED_SCRIPTS_PATH', ['browser/shared/scripts']);
    static sharedStylesPath = Router.resolveWorkspacePath('SHARED_STYLES_PATH', ['browser/shared/styles']);
    static middlewarePath = process.env.MIDDLEWARE_PATH || `${process.cwd()}/middleware`;
    ...
    static allMethods = process.env.ALLOW_METHODS
    ...
    static MAX_BODY_BYTES = Number(process.env.MAX_BODY_BYTES) || 1_048_576;
    /**
    ...
    static async validateRoute(route, staticPaths = []) {
        ...
        const routeMethod = route.split('/').pop();
        const { pathname: routePathname } = Router.validateSegmentPath(route, routeMethod, staticPaths);
        route = routePathname;
        const routeOptionsFile = Bun.file(`${Router.routesPath}${Router.routeToFilesystemPath(route)}/OPTIONS`);
    ...
    static async processRequest(request, route) {
        ...
        return {
            handler: `${Router.routesPath}${Router.routeToFilesystemPath(route)}/${request.method}`,
            stdin,
    ...
    static MAX_PARAM_LENGTH = Number(process.env.MAX_PARAM_LENGTH) || 1000;
    /**
