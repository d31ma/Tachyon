...
import { withPublicBrowserEnv } from "../server/browser-env.js";
...
const SHELLS_JSON_PATH = `${import.meta.dir}/../runtime/shell-manifest.json`;
...
export default class Tac {
    ...
    /** @type {OutputFormat} */
    static outputFormat = Tac.resolveOutputFormat();
    /** @type {string[]} */
    static mainEntryCandidates = ['main.js'];
    /** @type {string[]} */
    ...
    static createFilePathFilter(filePath) {
        ...
        return new RegExp(`^(?:${Tac.escapeRegExp(absolutePath)}|${Tac.escapeRegExp(posixPath)}|${Tac.escapeRegExp(fileUrl)})$`);
    }
    ...
    static isGlobalOutput() {
        return Tac.outputFormat === 'global';
    }
    ...
    static classifyElementTag(tagName) {
        ...
        if (Tac.compMapping.has(normalized))
            return { kind: 'component', name: normalized };
        if (Tac.isWebComponentTag(normalized))
            return { kind: 'web-component' };
        if (Tac.nativeTags.has(normalized))
            return { kind: 'native' };
        if (Tac.controlTags.has(normalized))
            return { kind: 'control' };
        return { kind: 'unknown' };
    }
    ...
    static isWebComponentTag(tagName) {
        return /^[a-z][.0-9_a-z]*-[.0-9_a-z-]*$/.test(tagName)
            && !Tac.reservedCustomElementNames.has(tagName);
    }
    ...
    static warnUnknownTag(tagName, sourceName) {
        ...
        if (Tac.warnedUnknownTags.has(key))
            return;
        Tac.warnedUnknownTags.add(key);
        Tac.compilerLogger.warn('Unknown element tag; treating it as plain HTML', {
            tag: tagName,
            source: sourceName,
        });
    }
    static async createStaticRoutes() {
        Tac.compMapping.clear();
        Tac.layoutMapping = {};
        Tac.warnedUnknownTags.clear();
        await Router.validatePageRoutes();
        await Tac.bundleBrowserRuntimeAssets();
        // JSON manifests
        ...
        await Tac.bundleComponents();
        await Tac.bundlePages();
        await Promise.all([
            Tac.bundleDependencies(),
            Tac.bundleAssets(),
            Tac.bundleSharedData()
        ]);
        // Write manifests after all routes are registered
        await Promise.all([
            Bun.write(Bun.file(ROUTES_JSON_PATH), JSON.stringify(Router.routeSlugs)),
            Bun.write(Bun.file(SHELLS_JSON_PATH), JSON.stringify(Tac.layoutMapping))
        ]);
    }
    ...
    static resolveLayout(pathname) {
        if (pathname !== '/') {
            ...
            for (let count = segments.length; count > 0; count -= 1) {
                ...
                const entry = Tac.layoutMapping[prefix];
                if (entry && (entry.allowSelf || prefix !== pathname))
                    return entry.path;
            }
        }
        const rootEntry = Tac.layoutMapping['/'];
    ...
    static isMainEntrypoint(filePath) {
        return Tac.mainEntryCandidates.includes(path.posix.basename(filePath));
    }
    static getMainEntryCandidates(cwd = process.cwd()) {
        const configuredSharedScriptsPath = process.env.SHARED_SCRIPTS_PATH;
        ...
        const candidates = [
            ...Tac.mainEntryCandidates.map((entry) => path.join(cwd, entry)),
            ...Tac.mainEntryCandidates.map((entry) => path.join(sharedScriptsBase, entry)),
        ];
        return [...new Set(candidates)];
    }
    static async getMainEntrypointPath(cwd = process.cwd()) {
        for (const candidate of Tac.getMainEntryCandidates(cwd)) {
    ...
    static getBrowserRuntimeRoutes() {
        return [...Tac.browserRuntimeRoutes];
    }
    ...
    static async getCompanionScriptPath(sourcePath) {
        const basePath = Tac.templateBasePath(sourcePath);
        for (const extension of Tac.companionScriptExtensions) {
    ...
    static async getCompanionStylePath(sourcePath) {
        const candidate = `${Tac.templateBasePath(sourcePath)}.css`;
    ...
    static buildCompanionStyle(css, dir, route) {
        if (dir !== 'components')
            return css;
        const scopeName = Tac.normalizeComponentName(route);
    ...
    static async transpileInlineScript(scriptContent, scriptLang) {
        if (!scriptContent)
            return '';
        const loader = Tac.normalizeScriptLoader(scriptLang);
        if (loader === 'js')
    ...
    static async renderShellHTML(options = {}) {
        ...
        shellHTML = shellHTML.replace('<!--__TACHYON_DEV_HEAD__-->', includeHotReloadClient
            ...
            : '');
        return withPublicBrowserEnv(Tac.withMainStylesheet(shellHTML));
    }
    static async bundleBrowserRuntimeAssets() {
        ...
        const mainEntrypoint = await Tac.getMainEntrypointPath();
        if (mainEntrypoint)
    ...
    static async renderPageDocument(distPath, pathname, shellHTML) {
        const pageFile = Tac.pageModuleFilePath(distPath, pathname);
        const pagePublicPath = Tac.pageModulePublicPath(pathname);
        await Tac.rewriteAbsoluteImports(pageFile, distPath);
        const layoutRoute = Tac.resolveLayout(pathname);
        const layoutFile = layoutRoute
            ? path.join(distPath, layoutRoute.slice(1))
            : null;
        if (layoutFile)
            await Tac.rewriteAbsoluteImports(layoutFile, distPath);
        const prerender = Tac.installPrerenderGlobals(distPath);
        try {
            let layoutHTML = '';
            if (layoutFile) {
                const layoutModule = await Tac.loadRenderFactoryForPrerender(layoutFile, /** @type {string} */ (layoutRoute));
                const layoutFactory = await layoutModule();
                layoutHTML = await layoutFactory();
            }
            const pageModule = await Tac.loadRenderFactoryForPrerender(pageFile, pagePublicPath);
            ...
            const bodyHTML = layoutHTML
                ...
                : pageHTML;
            const title = Tac.escapeHTML(prerender.title || Tac.routeTitleFallback);
            ...
            return `${Tac.normalizeScopedStyles(withBody)}\n`;
        }
        finally {
            prerender.restore();
        }
    ...
    static async renderPageDocumentForWorker(distPath, pathname, shellHTML, layoutMapping) {
        const previousLayoutMapping = Tac.layoutMapping;
        Tac.layoutMapping = { ...layoutMapping };
        try {
            return await Tac.renderPageDocument(distPath, pathname, shellHTML);
        }
        finally {
            Tac.layoutMapping = previousLayoutMapping;
        }
    }
    /** @param {string} distPath @param {string} pathname @param {string} shellHTML */
    static async renderPageDocumentIsolated(distPath, pathname, shellHTML) {
        ...
        proc.stdin.write(JSON.stringify({
            ...
            pathname,
            shellHTML,
            layoutMapping: Tac.layoutMapping,
        }));
        proc.stdin.end();
    ...
    static async prerenderStaticPages(distPath) {
        const htmlRoutes = Tac.getHtmlRoutes();
        await Tac.prerenderRoutes(distPath, htmlRoutes);
    }
    ...
    static async prerenderRoutes(distPath, routes) {
        const shellHTML = await Tac.renderShellHTML();
        /** @type {{ outputFile: string, html: string }[]} */
        const renderedRoutes = [];
        let renderIndex = 0;
        const renderWorkers = Array.from({ length: Math.min(Tac.prerenderRenderConcurrency, routes.length) }, async () => {
            while (renderIndex < routes.length) {
                ...
                const html = routes.length === 1
                    ? await Tac.renderPageDocument(distPath, route, shellHTML)
                    : await Tac.renderPageDocumentIsolated(distPath, route, shellHTML);
                renderedRoutes[currentIndex] = { outputFile, html };
            }
        });
        await Promise.all(renderWorkers);
        let index = 0;
        const workers = Array.from({ length: Math.min(Tac.prerenderWriteConcurrency, renderedRoutes.length) }, async () => {
    ...
    static async bundlePageFile(route) {
        ...
        const data = await Tac.extractComponents(await Bun.file(sourcePath).text(), sourcePath, 'pages', route);
        const moduleRoute = Tac.toModuleOutputRoute(route);
        const routePath = Tac.routePathFromPageSource(route);
        const publicPath = `/pages/${moduleRoute}`;
        await Tac.registerModule(data, moduleRoute, 'pages', sourcePath);
        if (data.hasSlot && !Tac.layoutMapping[routePath]?.allowSelf) {
            Tac.layoutMapping[routePath] = {
                path: publicPath,
                allowSelf: false,
            };
        }
    ...
    static async bundleComponentFile(comp) {
        ...
        const componentName = Tac.normalizeComponentName(comp);
        const modulePath = comp.replace('.html', '.js');
        const sourcePath = `${Router.componentsPath}/${comp}`;
        if (Tac.compMapping.has(componentName) && Tac.compMapping.get(componentName) !== modulePath) {
            throw new Error(`Duplicate component name '${componentName}' for '${comp}' and '${Tac.compMapping.get(componentName)}'`);
        }
        Tac.compMapping.set(componentName, modulePath);
        const data = await Tac.extractComponents(await Bun.file(sourcePath).text(), sourcePath, 'components', comp);
        await Tac.registerModule(data, comp, 'components', sourcePath);
    }
    /** @param {string} route */
    ...
    static async extractComponents(data, sourcePath, dir, route) {
        ...
        const companionScriptPath = await Tac.getCompanionScriptPath(sourcePath);
        const companionStylePath = await Tac.getCompanionStylePath(sourcePath);
        if (companionStylePath) {
            const css = await readFile(companionStylePath, 'utf8');
            const companionStyle = Tac.buildCompanionStyle(css, dir, route);
        ...
        return {
            ...
            companionImportPath: companionScriptPath
                ? Tac.toRelativeImportPath(sourcePath, companionScriptPath)
                : undefined
        };
    }
    // ── HTML → AST parsing ─────────────────────────────────────────────────────
        ...
        return new Promise((resolve) => {
            ...
            const interpolate = (text) => {
                ...
                return escapeTemplateLiteral(templated)
                    .replace(/__TY_EXPR_(\d+)__/g, (_match, index) => `\${ty_escapeText(${escapedExpressions[Number(index)]})}`)
            ...
            const rewriter = new HTMLRewriter()
                ...
                element(element) {
                    ...
                    for (const [name, value] of element.attributes)
                        attrs[name] = value;
                    const tagResolution = Tac.classifyElementTag(tagLower);
                    const resolvedComponent = tagResolution.kind === 'component' ? tagResolution.name : null;
                    if (tagResolution.kind === 'unknown') {
                        Tac.warnUnknownTag(tagLower, sourceName);
                    }
                    // Component import
                    if (resolvedComponent) {
                        const filepath = Tac.compMapping.get(resolvedComponent);
    ...
    static async createJSData(data, elements, publicPath) {
        ...
        renderSource = renderSource.replaceAll(/`<([A-Za-z0-9-]+)_\s*([\s\S]*?)\/>`/g, (_, component, attrStr) => {
            ...
            if (isLazy) {
                const filepath = Tac.compMapping.get(component);
                return `
                elements += \`<div id="${genId}" data-tac-scope="${component}" data-lazy-component="${component}" data-lazy-path="/components/${filepath}" data-lazy-props="\${${props.length ? `encodeURIComponent(JSON.stringify(${propsObj}))` : "''"}}\" ${events.join(' ')}></div>\`
                `;
            }
            return `
                elements += \`<div id="${genId}" data-tac-scope="${component}" ${events.join(' ')}>\`
        ...
        const rawScriptContent = await Tac.transpileInlineScript(data.script ?? '', data.scriptLang);
        const { bindingNames: dynamicImportBindings, moduleImports: dynamicModuleImports, scriptContent, } = Tac.liftDynamicImports(rawScriptContent);
...
const __ty_companion__ = await (async () => {
    ...
    if (__ty_instance__ && typeof __ty_instance__.__attachTacHelpers__ === 'function') {
        __ty_instance__.__attachTacHelpers__(__ty_runtime_bindings__);
    }
...
`;
    ...
    static createTemplatePlugin(options) {
        ...
        const filter = Tac.createFilePathFilter(sourcePath);
        const sourceLabel = Tac.toSourceLabel(sourcePath);
        return /** @type {BunPlugin} */ ({
            name: `tachyon-tac-template:${publicPath}`,
            target: /** @type {import("bun").Target} */ ('browser'),
            setup(build) {
                build.onLoad({ filter }, async () => {
                    const parsed = await Tac.parseHTML(data.html, `${dir}/${route} (${sourceLabel})`);
                    const baseCode = await Tac.createJSData(data, parsed, publicPath);
                    const jsCode = Tac.isGlobalOutput()
                        ? Tac.wrapGlobalModule(baseCode, publicPath)
                        : baseCode;
                    return {
                        contents: jsCode,
                        loader: 'js',
        ...
        return /** @type {BunPlugin} */ ({
            ...
            setup(build) {
                build.onLoad({ filter }, async () => {
                    ...
                    if (Tac.shouldInjectTacImport(contents)) {
                        const importPath = Tac.toRelativeImportPath(sourcePath, frameworkEntryPath);
                        contents = `import Tac from ${JSON.stringify(importPath)};\n${contents}`;
                    }
                    return {
                        contents,
                        loader: Tac.loaderForFilePath(sourcePath),
                    };
                });
            }
        });
    ...
    static async registerModule(data, route, dir, sourcePath) {
        const publicPath = `/${dir}/${Tac.toModuleOutputRoute(route)}`;
        if (Tac.isGlobalOutput()) {
            const parsed = await Tac.parseHTML(data.html, `${dir}/${route} (${Tac.toSourceLabel(sourcePath)})`);
            const baseCode = await Tac.createJSData(data, parsed, publicPath);
            const output = new Bun.Transpiler({ loader: 'js' })
                .transformSync(Tac.wrapGlobalModule(baseCode, publicPath));
        ...
        const result = await Bun.build({
            ...
            splitting: false,
            plugins: [
                ...(data.companionImportPath ? [Tac.createCompanionScriptPlugin(path.resolve(path.dirname(sourcePath), data.companionImportPath))] : []),
                Tac.createTemplatePlugin({
                    data,
                    dir,
                    route,
                    sourcePath,
    ...
    static async bundlePages() {
        if (await pathExists(Router.pagesPath)) {
            const routes = Array.from(new Bun.Glob(`**/${Tac.pageFileName}`).scanSync({ cwd: Router.pagesPath }));
            if (await Bun.file(path.join(Router.pagesPath, Tac.pageFileName)).exists() && !routes.includes(Tac.pageFileName))
                routes.unshift(Tac.pageFileName);
            for (const route of routes) {
                await Tac.bundlePageFile(route);
            }
        }
        // 404 page
        ...
        const nfData = await Tac.extractComponents(nfContent, nfSourcePath, 'pages', '404.html');
        await Tac.registerModule(nfData, '404.html', 'pages', nfSourcePath);
    }
    static async bundleComponents() {
        ...
        for (const comp of new Bun.Glob('**/*.html').scanSync({ cwd: Router.componentsPath })) {
            await Tac.bundleComponentFile(comp);
        }
    }
    static async bundleDependencies() {
        ...
        for (const mod of modules) {
            try {
                ...
                Tac.compilerLogger.warn('Failed to bundle dependency module', { module: mod, err: e });
            }
        }
    }
}
