...
//
// Parses a `yon`-shaped worker source - `class Handler { public: static
...
//
// Tac-C++ dialect (a documented subset, not full C++): integer aliases, `bool`,
// `string`, and `json`
// types, `let`-style declarations, assignment, arithmetic (`+ - * / %`),
// comparisons, the ternary operator `?:` for conditionals (C++ has no
...
export function tokenize(source) {
    ...
    while (i < source.length) {
        ...
        if (c === '"') {
            ...
            if (source[i] !== '"')
                throw new Error(`Tac worker (cpp): unterminated string literal on line ${line}`);
            i += 1;
        ...
        throw new Error(`Tac worker (cpp): unexpected character '${c}' on line ${line}`);
    }
    tokens.push({ type: 'eof', value: '', line });
    return tokens;
}

/** Recursive-descent parser for the Tac-C++ handler subset. */
export class Parser {
    /** @param {Token[]} tokens */
    constructor(tokens) {
        this.tokens = tokens;
    ...
    expectPunct(value) {
        ...
        if (t.type !== 'punct' || t.value !== value)
            throw new Error(`Tac worker (cpp): expected '${value}' but found '${t.value || 'end of input'}' on line ${t.line}`);
    }
    ...
    expectIdent() {
        const t = this.next();
        if (t.type !== 'ident')
            throw new Error(`Tac worker (cpp): expected an identifier but found '${t.value || 'end of input'}' on line ${t.line}`);
        return t.value;
    }
    ...
    parseProgram() {
        ...
        while (!this.isPunct('}')) {
            if (this.peek().type === 'eof')
                throw new Error('Tac worker (cpp): unexpected end of input inside the class body');
    ...
    parseValueType() {
        ...
        if ((t.type === 'kw' || t.type === 'ident') && (t.value === 'signed' || t.value === 'unsigned')) {
            ...
            throw new Error(`Tac worker (cpp): expected an integer type after '${t.value}' on line ${t.line}`);
        }
        ...
        throw new Error(`Tac worker (cpp): expected a type (integer, bool, string, or json) but found '${t.value || 'end of input'}' on line ${t.line}`);
    }
    ...
    parseBlock() {
        ...
        while (!this.isPunct('}')) {
            if (this.peek().type === 'eof')
                throw new Error('Tac worker (cpp): unexpected end of input inside a block');
    ...
    parseStatement() {
        ...
        if (this.isKw('if')) {
            throw new Error(`Tac worker (cpp): 'if' statements are not supported on line ${this.peek().line}; use the ternary operator '?:' for conditionals`);
        }
        ...
        if (t.type === 'ident' || (t.type === 'kw' && t.value === 'json')) {
            ...
            if (this.isPunct('.')) {
                ...
                if (memberToken.type !== 'ident' && !(memberToken.type === 'kw' && memberToken.value === 'json'))
                    throw new Error(`Tac worker (cpp): expected a member name but found '${memberToken.value || 'end of input'}' on line ${memberToken.line}`);
                ...
                throw new Error(`Tac worker (cpp): unsupported call '.${member}()' on line ${t.line} (request.len(), request.body(), and request.json() are available)`);
            }
            return { kind: 'var', name };
        }
        throw new Error(`Tac worker (cpp): unexpected token '${t.value || 'end of input'}' on line ${t.line}`);
    }
}
 ...
 * @returns {Uint8Array}
 */
export function compileCppWorker(source) {
