...
// A bump allocator is enough: a companion allocates a handful of small
// buffers per interaction and the module is discarded with the page.
...
pub extern "C" fn tac_invoke(ptr: i32, len: i32) -> i64 {
    ...
    let response = match (op.as_str(), name.as_str()) {
        // init declares the members, so the host never has to guess whether a
        // name is a field or a method.
        ...
        ("set", "count") => {
            let start = request.find("\"value\":").map(|i| i + 8).unwrap_or(0);
            let rest = &request[start..];
